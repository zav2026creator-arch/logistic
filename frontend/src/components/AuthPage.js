    ...
    <div className="min-h-screen bg-slate-100 flex items-center justify-center p-6">
      <div className="bg-white p-12 rounded-[50px] w-full max-w-2xl shadow-2xl">
        <h2 className="text-4xl font-black mb-8 italic uppercase tracking-tighter text-slate-800">
          {isReg ? 'Регистрация' : 'Вход в систему'}
        </h2>
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input required className="p-4 bg-slate-50 rounded-2xl outline-none font-bold" placeholder="Логин" onChange={e => setForm({...form, username: e.target.value})} />
          <input required className="p-4 bg-slate-50 rounded-2xl outline-none font-bold" type="password" placeholder="Пароль" onChange={e => setForm({...form, password: e.target.value})} />
          ...
          {isReg && (
            <>
              ...
              <input className="p-4 bg-slate-50 rounded-2xl border-none" placeholder="Организация" onChange={e => setForm({...form, org_name: e.target.value})} />
              <input className="md:col-span-2 p-4 bg-slate-50 rounded-2xl border-none" placeholder="Юр. адрес" onChange={e => setForm({...form, reg_address: e.target.value})} />
              <input className="p-4 bg-slate-50 rounded-2xl border-none" placeholder="ИНН / Рег. код" onChange={e => setForm({...form, reg_code: e.target.value})} />
            </>
          )}
