...
// Добавили иконку User в импорт для десктопного меню
import { Map as MapIcon, Truck, ShieldCheck, LogOut, User, ChevronDown } from 'lucide-react';
...
const Navbar = ({ user, onLogout }) => {
  ...
  return (
    <header className="sticky top-0 z-[3000] w-full bg-white/80 backdrop-blur-md border-b border-slate-100">
        ...
        <div className="flex justify-between items-center h-16 md:h-20">
          ...
          <div className="flex items-center gap-2 md:gap-8">
            ...
            <nav className="hidden md:flex items-center gap-1 ml-4">
              ...
              {/* ССЫЛКА НА ПРОФИЛЬ (ДЕСКТОП) */}
              <NavLink to="/profile" icon={<User size={16}/>} label={t('profile', 'Профиль')} active={isActive('/profile')} />
      ...
      <div className="md:hidden flex border-t border-slate-50 bg-white/90 backdrop-blur-md">
        <MobileNavLink to="/" icon={<MapIcon size={20}/>} active={isActive('/')} />
        <MobileNavLink to="/orders" icon={<Truck size={20}/>} active={isActive('/orders')} />
        
        {/* ССЫЛКА НА ПРОФИЛЬ (МОБИЛКИ) */}
        <MobileNavLink to="/profile" icon={<User size={20}/>} active={isActive('/profile')} />
